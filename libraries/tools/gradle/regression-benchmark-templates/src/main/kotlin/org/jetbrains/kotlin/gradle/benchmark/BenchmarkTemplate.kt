...
abstract class BenchmarkTemplate(
    ...
    fun aggregateBenchmarkResults(vararg benchmarkResults: BenchmarkResult) {
        ...
        val results = benchmarkResults
            .map { result ->
                DataFrame
                    ...
                    .groupBy("scenario").aggregate { // merging configuration and build times into one row
                        rows().first { it.values().contains("task start") }["median build time"] into "tasks start median time"
                        rows().first { it.values().contains("execution") }["median build time"] into "execution median time"
                    }
            ...
            .sortBy("scenario")
            .rename { column<String>("scenario") }.into("Scenario")
            .reorderColumnsBy {
